...
// Leaning towards grouping the application hooks into a single class
// possibly CRTP it
template <typename mp_int>
struct bigint_tommath_requires
{
  static bool copy(const mp_int *a, mp_int *b);
  static bool init_copy(mp_int *a, const mp_int *b);
  static bool init_size(mp_int *a, int size);
  static bool grow(mp_int *a, int size);
  static void clear(mp_int *a);
};
...
class bigint_tommath
{
 public:
  ...
  static inline mp_digit_const_iter mp_int_get_citer(const mp_int *i)
  {
    return i->dp;
  }
  ...
  // Implemented as calls into the application provided requires instance
  static mp_err mp_init_size(mp_int *a, int size)
  {
    return bigint_tommath_requires<mp_int>::init_size(a, size) ? MP_OKAY
                                                               : MP_MEM;
  }
...
#define MP_WARRAY \
  (int)(1uLL << (((8 * sizeof(mp_word)) - (2 * MP_DIGIT_BIT)) + 1))
...
#define MP_TOOM_SQR_CUTOFF MP_DEFAULT_TOOM_SQR_CUTOFF
  ...
  static mp_err mp_abs(const mp_int *a, mp_int *b)
  {
    mp_err err;
  ...
  static mp_err mp_add(const mp_int *a, const mp_int *b, mp_int *c)
  {
    mp_sign sa, sb;
    mp_err err;
  ...
  static mp_err mp_add_d(const mp_int *a, mp_digit b, mp_int *c)
  {
    mp_err err;
    int ix, oldused;
  ...
  static mp_err mp_and(const mp_int *a, const mp_int *b, mp_int *c)
  {
    int used = MP_MAX(a->used, b->used) + 1, i;
    mp_err err;
    mp_digit ac = 1, bc = 1, cc = 1;
    mp_sign csign =
        ((a->sign == MP_NEG) && (b->sign == MP_NEG)) ? MP_NEG : MP_ZPOS;
  ...
  static mp_err mp_init(mp_int *a)
  {
    /* special cased */
    /* equivalent to the normal definition, swapping it out removes an
     * allocation site */
    enum
    {
      MP_PREC = 8,
  ...
  static mp_ord mp_cmp(const mp_int *a, const mp_int *b)
  {
  ...
  static mp_ord mp_cmp_d(const mp_int *a, mp_digit b)
  {
    /* compare based on sign */
    if (a->sign == MP_NEG)
      {
  ...
  static mp_ord mp_cmp_mag(const mp_int *a, const mp_int *b)
  {
    int n;
  ...
  static mp_err mp_complement(const mp_int *a, mp_int *b)
  {
    mp_err err = mp_neg(a, b);
    return (err == MP_OKAY) ? mp_sub_d(b, 1uL, b) : err;
  }
  static mp_err mp_copy(const mp_int *a, mp_int *b)
  {
    int n;
    mp_err err;
  ...
  static int mp_count_bits(const mp_int *a)
  {
    int r;
    mp_digit q;
  ...
  static mp_err mp_decr(mp_int *a)
  {
    if (MP_IS_ZERO(a))
      {
        mp_set(a, 1uL);
  ...
  static mp_err mp_div(const mp_int *a, const mp_int *b, mp_int *c, mp_int *d)
  {
    mp_int ta, tb, tq, q;
    int n, n2;
    mp_err err;
  ...
  static mp_err mp_div_2(const mp_int *a, mp_int *b)
  {
    int x, oldused;
    mp_digit r, rr;
    mp_err err;
  ...
  static mp_err mp_div_2d(const mp_int *a, int b, mp_int *c, mp_int *d)
  {
    mp_digit D, r, rr;
    int x;
    mp_err err;
  ...
  static mp_err mp_div_d(const mp_int *a, mp_digit b, mp_int *c, mp_digit *d)
  {
    mp_int q;
    mp_word w;
    mp_digit t;
  ...
  static double mp_get_double(const mp_int *a)
  {
    int i;
  ...
  static mp_err mp_incr(mp_int *a)
  {
    if (MP_IS_ZERO(a))
      {
        mp_set(a, 1uL);
  ...
  static mp_err mp_lshd(mp_int *a, int b)
  {
    int x;
    mp_err err;
  ...
  static mp_err mp_mod(const mp_int *a, const mp_int *b, mp_int *c)
  {
    mp_int t;
    mp_err err;
  ...
  static mp_err mp_mod_2d(const mp_int *a, int b, mp_int *c)
  {
    int x;
    mp_err err;
  ...
  static mp_err mp_mod_d(const mp_int *a, mp_digit b, mp_digit *c)
  {
    return mp_div_d(a, b, NULL, c);
  }
  static mp_err mp_mul(const mp_int *a, const mp_int *b, mp_int *c)
  {
    mp_err err;
    int min_len = MP_MIN(a->used, b->used), max_len = MP_MAX(a->used, b->used),
        digs = a->used + b->used + 1;
    mp_sign neg = (a->sign == b->sign) ? MP_ZPOS : MP_NEG;

    if (MP_HAS(S_MP_BALANCE_MUL) &&
        /* Check sizes. The smaller one needs to be larger than the Karatsuba
         * cut-off. The bigger one needs to be at least about one
         * MP_KARATSUBA_MUL_CUTOFF bigger to make some sense, but it depends on
         * architecture, OS, position of the stars... so YMMV. Using it to cut
         * the input into slices small enough for fast_s_mp_mul_digs was
         * actually slower on the author's machine, but YMMV.
         */
        ...
        (max_len >= (2 * min_len)))
      {
        ...
        err = s_mp_karatsuba_mul(a, b, c);
              ...
              */
             (digs < MP_WARRAY) && (min_len <= MP_MAXFAST))
      {
  ...
  static mp_err mp_mul_2(const mp_int *a, mp_int *b)
  {
    int x, oldused;
    mp_err err;
  ...
  static mp_err mp_mul_2d(const mp_int *a, int b, mp_int *c)
  {
    mp_digit d;
    mp_err err;
  ...
  static mp_err mp_mul_d(const mp_int *a, mp_digit b, mp_int *c)
  {
    mp_digit u;
    mp_word r;
    mp_err err;
  ...
  static mp_err mp_neg(const mp_int *a, mp_int *b)
  {
    mp_err err;
    if (a != b)
      {
  ...
  static mp_err mp_or(const mp_int *a, const mp_int *b, mp_int *c)
  {
    int used = MP_MAX(a->used, b->used) + 1, i;
    mp_err err;
    mp_digit ac = 1, bc = 1, cc = 1;
  ...
  static
      ...
      mp_err
      mp_set_double(mp_int *a, double b)
  {
    uint64_t frac;
    int exp;
    mp_err err;
...
#else
/* pragma message() not supported by several compilers (in mostly older but
 * still used versions) */
#ifdef _MSC_VER
#pragma message( \
...
#warning \
...
#endif
#endif
  static MP_SET_UNSIGNED(mp_set_u64, uint64_t) static mp_err
      mp_signed_rsh(const mp_int *a, int b, mp_int *c)
  {
    mp_err res;
    if (a->sign == MP_ZPOS)
      {
  ...
  static mp_err mp_sqr(const mp_int *a, mp_int *b)
  {
    mp_err err;
#if 0
  ...
  static mp_err mp_sqrt(const mp_int *arg, mp_int *ret)
  {
    mp_err err;
    mp_int t1, t2;
  ...
  static mp_err mp_sub(const mp_int *a, const mp_int *b, mp_int *c)
  {
  ...
  static mp_err mp_sub_d(const mp_int *a, mp_digit b, mp_int *c)
  {
    mp_err err;
    int ix, oldused;
  ...
  static mp_err mp_xor(const mp_int *a, const mp_int *b, mp_int *c)
  {
    int used = MP_MAX(a->used, b->used) + 1, i;
    mp_err err;
    mp_digit ac = 1, bc = 1, cc = 1;
 ...
 private:
  static mp_err s_mp_add(const mp_int *a, const mp_int *b, mp_int *c)
  {
    const mp_int *x;
    mp_err err;
    int olduse, min, max;
  ...
  static mp_err s_mp_balance_mul(const mp_int *a, const mp_int *b, mp_int *c)
  {
  ...
  static mp_err s_mp_div_3(const mp_int *a, mp_int *c, mp_digit *d)
  {
    mp_int q;
    mp_word w, t;
    mp_digit b;
  ...
  static mp_err s_mp_mul_digs(const mp_int *a, const mp_int *b, mp_int *c,
                              int digs)
  {
    mp_int t;
    mp_err err;
  ...
  static mp_err s_mp_mul_digs_fast(const mp_int *a, const mp_int *b, mp_int *c,
                                   int digs)
  {
    int olduse, pa, ix, iz;
    mp_err err;
  ...
  static mp_err s_mp_sqr(const mp_int *a, mp_int *b)
  {
    mp_int t;
    int ix, iy, pa;
    mp_err err;
  ...
  static mp_err s_mp_sqr_fast(const mp_int *a, mp_int *b)
  {
    int olduse, pa, ix, iz;
    mp_digit W[MP_WARRAY];
    mp_word W1;
  ...
  static mp_err s_mp_sub(const mp_int *a, const mp_int *b, mp_int *c)
  {
    int olduse, min, max;
    mp_err err;
